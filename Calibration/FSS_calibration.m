...
Data.FSS = lowpass(Data.FSS, 5, 1/0.01);
...
%% Data processing
Data_i.FSS = Data_i.FSS-mean(Data_i.FSS(500:1000,:));
Data_i.FSS = max(Data_i.FSS,0.0001);
...
cell_fit_linear = fit(Data_i.FSS(500:end-1000,cell_num),-Data_i.FT_r_xy(500:end-1000,1), ft_linear, 'StartPoint',[0.5, 0.8], 'Robust','LAR','MaxIter',10000);

cali_data_linear = cell_fit_linear(Data_i.FSS(:,cell_num));

%Piecewise Fitting
ft_piece = fittype('a0*x^3+a1*x','indep','x');
%ft_piece = fittype('a0*sqrt(x)+a2*x^2');
cell_fit_piece = fit(Data_i.FSS(500:end-1000,cell_num),-Data_i.FT_r_xy(500:end-1000,1), ft_piece, 'StartPoint',[2, 2], 'Robust','LAR','MaxIter',10000);
cali_data_piece = cell_fit_piece(Data_i.FSS(:,cell_num));
...
figure(2)
clf
plot(movmean(Data_i.FSS(:,cell_num),100),movmean(-Data_i.FT_r_xy(:,1),100),'.')
title('Tactile vs. FT')
...
title('Linear');
clf
plot(cell_fit_linear, Data_i.FSS(500:end-1000,cell_num),-Data_i.FT_r_xy(500:end-1000,1))
xlabel('Tactile sensor')
ylabel('FT sensor')
% axis equal
grid on
...
title('Piece');
clf
plot(cell_fit_piece, Data_i.FSS(500:end-1000,cell_num),-Data_i.FT_r_xy(500:end-1000,1))
xlabel('Tactile sensor')
ylabel('FT sensor')
% axis equal
grid on

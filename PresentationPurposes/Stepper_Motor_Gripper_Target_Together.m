...
%% Bag Read
varname = strings;
filename = "rosbag/0721StepMotor.bag";
bag = rosbag(filename);
k = 1;
...
for i = Initial:10:Final
    ...
    xline(i-Initial, 'r')
    hold off
    title('FSS Data of Gripper and Target Object Moving Together', 'FontSize', 12)
    ylabel('FSS Signal')
    xlabel('Timestep')
    ...
    ylim([0 200000])
    grid on
    title('FFT Data of Gripper and Target Object Moving Together', 'FontSize', 12)
    ylabel('FFT Amplitude')
    xlabel('Sample Frequency(Hz)')
    if(abs(Data_i.HCmotor(i))==10)
        text(9, 140000, 'Gripper Moving with Object', 'Color', 'red', 'FontSize', 20)
    ...
    filename_ = 'Step_Gipper_and_Target.gif';
    del = 0.1;
    if(i == Initial)
